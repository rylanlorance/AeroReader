...
class TextCluster(BaseInterface):
    ...
    def __init__(self, book: str, n_clusters: int = 15):
        ...
        cleaner = CleanText()
        pages = GetText.extract_all(book, cleaner)
    ...
    def tree_analysis(self):
        ...
        raw_pages = GetText.extract_all(self.book, lambda text: text)

        # Attach page numbers of 10 most emblematic pages in cluster
        # Attach emblematic page text
        for i, row in enumerate(scores[:, ::-1]):
            self.clusters[i].page_numbers = self.used[row[0, :10]].tolist()[0]

            j = 0
            while self.tree.labels_[row[0, j]] != i:
                j += 1

            self.clusters[i].example_page = raw_pages[self.used[row[0, j]]]
